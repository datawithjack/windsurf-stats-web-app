...
router.get('/', async (req, res) => {
    try {
        ...
        let query = `SELECT MAX(total_wave) as best_wave_score, sailor_name FROM PWA_HEAT_DATA`;
        const params = [];
        if (athlete && typeof athlete === 'string') {
            query += ' WHERE sailor_name = ?';
            params.push(athlete);
        }
        query += ' GROUP BY sailor_name ORDER BY best_wave_score DESC';
...
//# sourceMappingURL=best-wave-score.js.map