...
router.get('/', async (req, res) => {
    try {
        ...
        let query = `SELECT MAX(total_points) as best_heat_score, sailor_name FROM PWA_HEAT_DATA`;
        const params = [];
        if (athlete && typeof athlete === 'string') {
            query += ' WHERE sailor_name = ?';
            params.push(athlete);
        }
        query += ' GROUP BY sailor_name ORDER BY best_heat_score DESC';
...
//# sourceMappingURL=best-heat-score.js.map